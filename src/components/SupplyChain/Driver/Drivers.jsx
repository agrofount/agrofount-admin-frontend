...
import DriverItemTable from "./DriverItemTable";
...
const ListDrivers = () => {
  ...
  const fetchDrivers = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`${backend_url}/supply-chain/drivers`, {
      ...
      if (response.status === 200) setDrivers(response.data);
    } catch (error) {
      console.error("an error occurred: ", error);
    } finally {
      setIsLoading(false);
    }
  }, [backend_url, token, driverPage, pageLimit, searchValue, driverAdded]);
  ...
  }, [fetchDrivers, itemDeleted]);

  let tableContent;
  if (isLoading) {
    tableContent = <TableSkeleton />;
  } else if (drivers.data.length < 1) {
    tableContent = (
      <tbody>
        <tr>
          <td colSpan="12">
            <div className="flex justify-center items-center h-[300px] w-full">
              <div className="flex flex-col">
                ...
                <p className="text-[#ADADAD] text-sm mt-5">No Drivers yet</p>
              </div>
            </div>
          </td>
        </tr>
        ...
        <div className="flex flex-row justify-between items-center py-3">
          <div className="flex flex-row items-start gap-2">
            ...
            <p className="text-[#6E6E6E] font-roboto text-[15px] font-normal leading-normal">
              Tip search by Driver ID: Each product is provided with a unique
        ...
        {drivers.meta && (
          <div className="flex flex-row text-center justify-between my-5">
            <p className="text-sm text-gray-500">
              Showing {drivers.meta.currentPage} to {drivers.meta.totalPages} of{" "}
              {drivers.meta.totalItems} entries
            </p>

            <div className="flex flex-row">
              <button
                className={`flex flex-row px-3 py-1 mx-1 border font-normal rounded-full ${
                  Number(drivers.meta.currentPage) === 1
                    ? " bg-[#D5D5D5] text-white cursor-not-allowed"
                    : "bg-white "
                }`}
                onClick={() =>
                  handlePageChange(Number(drivers.meta.currentPage) - 1)
                }
                disabled={Number(drivers.meta.currentPage) === 1}
              >
                <FontAwesomeIcon
                  icon={faChevronLeft}
                  size="5rem"
              ...
              {[...Array(drivers.meta.totalPages)].map((_, index) => (
                <button
                  key={index}
                  className={`px-3 py-1 mx-1 border rounded-full ${
                    Number(drivers.meta.currentPage) === index + 1
              ...
              <button
                className={`flex flex-row  px-3 p-1 mx-1 border font-normal rounded-full ${
                  Number(drivers.meta.currentPage) === drivers.meta.totalPages
                    ? "bg-[#D5D5D5] text-white cursor-not-allowed "
                    : "bg-white"
                }`}
                onClick={() =>
