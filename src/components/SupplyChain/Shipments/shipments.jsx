...
const ListShipments = () => {
  ...
  const [shipments, setShipments] = useState({ data: [], meta: {} });
  ...
  } else if (shipments.data.length < 1) {
    tableContent = (
      <tbody>
        <tr>
          <td colSpan="12">
            <div className="flex justify-center items-center h-[300px] w-full">
              <div className="flex flex-col">
                ...
                <p className="text-[#ADADAD] text-sm mt-5">No Shipment yet</p>
              </div>
            </div>
          </td>
        </tr>
    ...
    tableContent = (
      <tbody>
        {shipments.data.map((shipment, index) => (
          <ShipmentItemTable shipment={shipment} key={index} />
        ))}
      </tbody>
    );
  }
    ...
    <div>
      <div className="flex flex-row justify-between items-center gap-5">
        ...
        <div className="flex flex-row items-center gap-2">
          ...
          <Link to="/supply-chain/shipments">
            <p className="text-[#6E6E6E] font-roboto text-[13px] font-normal leading-normal tracking-[0.26px]">
              Shipment List
            </p>
          </Link>
        ...
        <div className="flex flex-row justify-between items-center py-3">
          <div className="flex flex-row items-start gap-2">
            ...
            <p className="text-[#6E6E6E] font-roboto text-[15px] font-normal leading-normal">
              Tip search by Shipment ID: Each product is provided with a unique
        ...
        {shipments.meta && (
          <div className="flex flex-row text-center justify-between my-5">
            <p className="text-sm text-gray-500">
              Showing {shipments.meta.currentPage} to{" "}
              {shipments.meta.totalPages} of {shipments.meta.totalItems} entries
            </p>

            <div className="flex flex-row">
              <button
                className={`flex flex-row px-3 py-1 mx-1 border font-normal rounded-full ${
                  Number(shipments.meta.currentPage) === 1
                    ? " bg-[#D5D5D5] text-white cursor-not-allowed"
                    : "bg-white "
                }`}
                onClick={() =>
                  handlePageChange(Number(shipments.meta.currentPage) - 1)
                }
                disabled={Number(shipments.meta.currentPage) === 1}
              >
                <FontAwesomeIcon
                  icon={faChevronLeft}
                  size="5rem"
              ...
              {[...Array(shipments.meta.totalPages)].map((_, index) => (
                <button
                  key={index}
                  className={`px-3 py-1 mx-1 border rounded-full ${
                    Number(shipments.meta.currentPage) === index + 1
              ...
              <button
                className={`flex flex-row  px-3 p-1 mx-1 border font-normal rounded-full ${
                  Number(shipments.meta.currentPage) ===
                  shipments.meta.totalPages
                    ? "bg-[#D5D5D5] text-white cursor-not-allowed "
                    : "bg-white"
                }`}
                onClick={() =>
