...
const ListCountries = () => {
  ...
  const [countries, setCountries] = useState({ data: [], meta: {} });
        ...
        <td colSpan="12">
          <div className="flex justify-center items-center h-[300px] w-full">
            <div className="flex flex-col">
              ...
              <p className="text-[#ADADAD] text-sm mt-5">No country yet</p>
            </div>
          </div>
        </td>
      </tr>
        ...
        <div className="flex flex-row justify-between items-center py-3">
          <div className="flex flex-row items-start gap-2">
            ...
            <p className="text-[#6E6E6E] font-roboto text-[15px] font-normal leading-normal">
              Tip search by Country ID: Each product is provided with a unique
        ...
        {countries.meta && (
          <div className="flex flex-row text-center justify-between my-5">
            <p className="text-sm text-gray-500">
              Showing {countries.meta.currentPage} to{" "}
              {countries.meta.totalPages} of {countries.meta.totalItems} entries
            </p>

            <div className="flex flex-row">
              <button
                className={`flex flex-row px-3 py-1 mx-1 border font-normal rounded-full ${
                  Number(countries.meta.currentPage) === 1
                    ? " bg-[#D5D5D5] text-white cursor-not-allowed"
                    : "bg-white "
                }`}
                onClick={() =>
                  handlePageChange(Number(countries.meta.currentPage) - 1)
                }
                disabled={Number(countries.meta.currentPage) === 1}
              >
                <FontAwesomeIcon
                  icon={faChevronLeft}
                  size="5rem"
              ...
              {[...Array(countries.meta.totalPages)].map((_, index) => (
                <button
                  key={index}
                  className={`px-3 py-1 mx-1 border rounded-full ${
                    Number(countries.meta.currentPage) === index + 1
              ...
              <button
                className={`flex flex-row  px-3 p-1 mx-1 border font-normal rounded-full ${
                  Number(countries.meta.currentPage) ===
                  countries.meta.totalPages
                    ? "bg-[#D5D5D5] text-white cursor-not-allowed "
                    : "bg-white"
                }`}
                onClick={() =>
