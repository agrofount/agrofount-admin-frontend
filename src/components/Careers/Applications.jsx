...
const statusOptions = ["All Statuses", "New", "In Review", "Shortlisted", "Rejected", "Hired"];
const statusValues = {
  New: "new",
  "In Review": "reviewing",
...
const statusTone = {
  ...
  "In Review": "bg-[#e5f1ff] text-[#1677d2]",
  Shortlisted: "bg-[#efe6ff] text-[#7c3fd3]",
  Rejected: "bg-[#ffe4e4] text-[#ef3340]",
  Hired: "bg-[#dcf8e6] text-[#008f45]",
};
...
const Applications = () => {
  ...
  const loadApplications = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetchCareerApplications(
        buildCareerParams({
          page,
          limit: pageSize,
          search: searchValue,
      ...
      setApplications((response.data || []).map(normalizeApplication));
  ...
  const totalItems = Number(meta.totalItems || applications.length || 0);
    ...
    <div className="space-y-5 text-[#101828]">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs font-medium text-[#667085]">
            Review and track all career applications.
          </p>
        </div>
      ...
      <section className="overflow-hidden rounded-lg border border-[#e5e7eb] bg-white shadow-[0_8px_24px_rgba(16,24,40,0.04)]">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[920px] text-left text-xs">
            <thead className="bg-[#f8fafc] uppercase text-[#667085]">
              <tr>
                {["Applicant", "Email", "Phone", "Job", "Location", "Experience", "Status", "Submitted"].map((heading) => (
            ...
            <tbody>
              {loading ? (
                ...
                applications.map((application) => (
                  <tr key={application.id} className="border-b border-[#eef2f6] last:border-0 hover:bg-[#fbfcfd]">
                    <td className="px-4 py-3 font-semibold">{application.name}</td>
                    <td className="px-4 py-3">{application.email}</td>
                    <td className="px-4 py-3">{application.phoneNumber}</td>
                    <td className="px-4 py-3">{application.role}</td>
                    <td className="px-4 py-3">{[application.city, application.state].filter(Boolean).join(", ") || "N/A"}</td>
                    <td className="px-4 py-3">{application.yearsOfExperience || 0} yrs</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${statusTone[application.status] || "bg-[#f2f4f7] text-[#667085]"}`}>
        ...
        <div className="flex flex-col gap-4 border-t border-[#eef2f6] px-5 py-4 text-sm text-[#667085] md:flex-row md:items-center md:justify-between">
          <p>Showing {shownFrom} to {shownTo} of {totalItems} applications</p>
