...
const ListCities = () => {
  ...
  const [cities, setCities] = useState({ data: [], meta: {} });
    ...
    try {
      ...
      const response = await axios.get(`${backend_url}/city`, {
        ...
        params: {
          ...
          limit: pageLimit,
          search: searchValue,
          "filter.state.id": stateId,
        },
      });
      console.log("this is the city", response.data);
  ...
  }, [fetchCities, itemDeleted, stateId]);
          ...
          <td colSpan="12">
            <div className="flex justify-center items-center h-[300px] w-full">
              <div className="flex flex-col">
                ...
                <p className="text-[#ADADAD] text-sm mt-5">No City yet</p>
              </div>
            </div>
          </td>
        </tr>
        ...
        <div className="flex flex-row justify-between items-center py-3">
          <div className="flex flex-row items-start gap-2">
            ...
            <p className="text-[#6E6E6E] font-roboto text-[15px] font-normal leading-normal">
              Tip search by City ID: Each product is provided with a unique ID,
              which you can rely on to find the exact product you need.
            </p>
          </div>
        </div>

        <div className="flex flex-col sm:flex-row justify-between gap-2 items-center py-3">
          <div className="flex flex-row items-start gap-2">
          ...
          <div className="w-full mx-3 sm:w-1/6 px-auto">
            <Link
              to={`/cities/add?countryId=${countryId}&stateId=${stateId}`}
              className="w-full flex flex-row gap-2 border border-[#61BF75] px-1 py-1 sm:px-10 sm:py-3 rounded-lg sm:rounded-full cursor-pointer"
            >
        ...
        {cities.meta && (
          <div className="flex flex-row text-center justify-between my-5">
            <p className="text-sm text-gray-500">
              Showing {cities.meta.currentPage} to {cities.meta.totalPages} of{" "}
              {cities.meta.totalItems} entries
            </p>

            <div className="flex flex-row">
              <button
                className={`flex flex-row px-3 py-1 mx-1 border font-normal rounded-full ${
                  Number(cities.meta.currentPage) === 1
                    ? " bg-[#D5D5D5] text-white cursor-not-allowed"
                    : "bg-white "
                }`}
                onClick={() =>
                  handlePageChange(Number(cities.meta.currentPage) - 1)
                }
                disabled={Number(cities.meta.currentPage) === 1}
              >
                <FontAwesomeIcon
                  icon={faChevronLeft}
                  size="5rem"
              ...
              {[...Array(cities.meta.totalPages)].map((_, index) => (
                <button
                  key={index}
                  className={`px-3 py-1 mx-1 border rounded-full ${
                    Number(cities.meta.currentPage) === index + 1
              ...
              <button
                className={`flex flex-row  px-3 p-1 mx-1 border font-normal rounded-full ${
                  Number(cities.meta.currentPage) === cities.meta.totalPages
                    ? "bg-[#D5D5D5] text-white cursor-not-allowed "
                    : "bg-white"
                }`}
                onClick={() =>
