...
const CountryTableItem = ({ country, setItemDeleted }) => {
  const [open, setOpen] = useState(false);
  const { backend_url, token } = useContext(ShopContext);

  const handleDelete = async () => {
    try {
      await axios.delete(`${backend_url}/country/${country.id}`, {
        headers: {
  ...
  return (
    <tr className="hover:bg-[#F7F7F7]">
      <td className="px-2 py-5 whitespace-nowrap text-sm text-gray-500">
        <p className="py-1">{country.id.slice(0, 10)}</p>
      </td>
      <td className="px-2 py-5 whitespace-nowrap text-sm text-gray-500">
      ...
      <td className="px-2 py-5 whitespace-nowrap text-sm text-gray-500">
        <p className="py-1">
          <p className="py-1">{country.code}</p>
        </p>
      </td>

      <td className="px-2 py-5 whitespace-nowrap text-sm text-gray-500">
        <p className="py-1">{new Date(country.createdAt).toDateString()}</p>
      </td>

      <td className="px-2 py-5 whitespace-nowrap text-sm text-gray-500">
        <p
          className={`py-1 ${
            country.isActive ? "bg-[#61BF75]" : "bg-[#F96767]"
      ...
      <td className="px-4 py-5 whitespace-nowrap text-sm text-gray-500">
        <div className="flex flex-row gap-3 items-center">
          ...
          <Link
            to={`/states/?countryName=${encodeURIComponent(
              country.name
                      ...
                      <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div className="mt-2">
                          <p className="text-sm text-gray-500">
                            Are you sure you want to delete your {country.name}?
