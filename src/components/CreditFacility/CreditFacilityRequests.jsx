...
import CreditFacilityRequestItem from "./CreditFaclityRequestItem";
...
const CreditFacilityRequests = () => {
    ...
    const fetchRequests = async () => {
      try {
        ...
        const response = await axios.get(
          `${backend_url}/credit-facility/requests`,
          {
        ...
        console.error("Error fetching credit facility requests:", e);
        toast.error(
          e.response?.data?.message ||
            "Failed to load credit facility requests."
        );
      } finally {
        setIsLoading(false);
      }
  ...
  } else if (requests.data.length < 1) {
    tableContent = (
    ...
    <div>
      <div className="flex flex-row justify-between items-center gap-5">
        ...
        <div className="flex flex-row items-center gap-2">
          ...
          <Link to="/credit-facility/requests">
            <p className="text-[#6E6E6E] font-roboto text-[13px] font-normal leading-normal tracking-[0.26px]">
              Payment List
            </p>
          </Link>
        ...
        <div className="flex flex-row justify-between items-center py-3">
          <div className="flex flex-row items-start gap-2">
            ...
            <p className="text-[#6E6E6E] font-roboto text-[15px] font-normal leading-normal">
              Tip search by Request ID: Each request is provided with a unique
              ID, which you can rely on to find the exact request you need.
            </p>
          </div>
        </div>
        ...
        {requests.meta && (
          <div className="flex flex-row text-center justify-between my-5">
            <p className="text-sm text-gray-500">
              Showing {requests.meta.currentPage} to {requests.meta.totalPages}{" "}
              of {requests.meta.totalItems} entries
            </p>

            <div className="flex flex-row">
              <button
                className={`flex flex-row px-3 py-1 mx-1 border font-normal rounded-full ${
                  Number(requests.meta.currentPage) === 1
                    ? " bg-[#D5D5D5] text-white cursor-not-allowed"
                    : "bg-white "
                }`}
                onClick={() =>
                  handlePageChange(Number(requests.meta.currentPage) - 1)
                }
                disabled={Number(requests.meta.currentPage) === 1}
              >
                <FontAwesomeIcon
                  icon={faChevronLeft}
                  size="5rem"
              ...
              {[...Array(requests.meta.totalPages)].map((_, index) => (
                <button
                  key={index}
                  className={`px-3 py-1 mx-1 border rounded-full ${
                    Number(requests.meta.currentPage) === index + 1
              ...
              <button
                className={`flex flex-row  px-3 p-1 mx-1 border font-normal rounded-full ${
                  Number(requests.meta.currentPage) === requests.meta.totalPages
                    ? "bg-[#D5D5D5] text-white cursor-not-allowed "
                    : "bg-white"
                }`}
                onClick={() =>
