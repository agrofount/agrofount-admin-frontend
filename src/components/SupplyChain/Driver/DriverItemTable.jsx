...
const DriverItemTable = ({ driver, setItemDeleted }) => {
  const [open, setOpen] = useState(false);
  const { backend_url, token } = useContext(ShopContext);

  const handleDelete = async () => {
    try {
      await axios.delete(`${backend_url}/supply-chain/drivers/${driver.id}`, {
        headers: {
  ...
  return (
    <tr className="hover:bg-[#F7F7F7]">
      <td className="px-2 py-5 whitespace-nowrap text-sm text-gray-500">
        <p className="py-1">{driver.name}</p>
      </td>
      <td className="px-2 py-5 whitespace-nowrap text-sm text-gray-500">
        <div className="flex flex-row items-start gap-4 max-w-32">
          <p className="py-1 text-wrap">{driver.phone}</p>
        </div>
      </td>
      ...
      <td className="px-2 py-5 whitespace-nowrap text-sm text-gray-500">
        <p className="py-1">
          <p className="py-1">{driver.licenseNumber}</p>
        </p>
      </td>

      <td className="px-2 py-5 whitespace-nowrap text-sm text-gray-500">
        <p className="py-1">
          <p className="py-1">{driver.mainLocation}</p>
        </p>
      </td>

      <td className="px-2 py-5 whitespace-nowrap text-sm text-gray-500">
        <p className="py-1">{new Date(driver.createdAt).toDateString()}</p>
      </td>

      <td className="px-4 py-5 whitespace-nowrap text-sm text-gray-500">
        <div className="flex flex-row gap-3 items-center">
          <Link to={`/users/${driver.id}/edit`}>
                      ...
                      <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div className="mt-2">
                          <p className="text-sm text-gray-500">
                            Are you sure you want to delete your {driver.name}?
